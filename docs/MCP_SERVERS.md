...
- Lifecycle: Managed by execution/models/anthropic_mcp_model.py
...
Each server has unit tests:
```bash
pytest mcp-server/test_anthropic_mcp_server.py -v
pytest mcp-server/test_openai_mcp_server.py -v
# etc.
```

### Integration Tests

Test complete workflows with each provider:
```bash
hillstar run examples/test_providers/anthropic_test.json
hillstar run examples/test_providers/openai_test.json
# etc.
```
...
```bash
hillstar test-mcp-connectivity
```
...
See PROVIDERS.md for provider-specific details and capabilities.

...
class AutoDiscover:
	...
	def get_project_info(start_dir: str = ".") -> Dict[str, Any]:
		...
		schema_path = os.path.join(
			current_dir, "python/hillstar/schemas/workflow-schema.json"
		)
		...
		if not has_schema:
			try:
				...
				schema_path = (
					"hillstar.schemas.workflow-schema.json (installed package)"
				)
			except Exception:
				pass
	...
	def suggest_workflows(
		...
		for workflow in workflows:
			...
			id_match = AutoDiscover._similarity_score(
				task_description, workflow.get("id", "")
			)
			score += id_match * 0.3

			# Match against workflow description
			desc = workflow.get("description", "")
			desc_match = AutoDiscover._similarity_score(task_description, desc)
	...
	def get_recommendations(
		...
		workflow_suggestions = AutoDiscover.suggest_workflows(
			task_description, workflows, top_k=3
		)
		...
		if preset_suggestions:
			...
			recommendation_parts.append(f"Suggested preset: {best_preset[0]}")

		recommendation_text = ". ".join(recommendation_parts)
		if not recommendation_text:
			recommendation_text = (
				"No specific recommendations. You may use any available preset."
			)

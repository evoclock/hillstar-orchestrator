...
class NodeExecutor:
    ...
    def _normalize_temperature_for_provider(
        self, provider: str, temperature: float
    ) -> float:
    ...
    def _get_provider_chain(self, node_id: str, node: dict) -> list:
        ...
        if config_prefs:
            config_preferred = self.model_factory.resolve_provider_preference(
                config_prefs
            )
    ...
    def _execute_model_call(self, node_id: str, node: dict, inputs: Any) -> dict:
        ...
        for attempt, provider in enumerate(provider_chain):
            ...
            model = self.model_factory.get_model(
                provider_to_use, model_name, **model_kwargs
            )
            ...
            if temperature is None:
                temperature = sampling_params.get(
                    "temperature", ModelSelector.get_temperature()
                )

            # Adjust temperature for provider constraints
            temperature = self._normalize_temperature_for_provider(
            ...
            if isinstance(result, dict) and result.get("error"):
                ...
                if (
                    self._is_fallback_error(error_msg)
                    and attempt < len(provider_chain) - 1
                ):
            ...
            if self.model_config.get("explainability", {}).get(
                "log_selection_reasoning"
            ):
                selection_log["selection_mode"] = self.model_config.get(
                    "mode", "explicit"
                )
    ...
    def _execute_git_commit(self, node_id: str, node: dict, inputs: Any) -> dict:
        ...
        author_email = node.get("parameters", {}).get(

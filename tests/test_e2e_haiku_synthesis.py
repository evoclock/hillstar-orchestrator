...
class TestHaikuSynthesisPipeline:
	...
	def workflow(self):
		...
		workflow_file = (
			Path(__file__).parent.parent / "examples" / "multi-provider-workflow.json"
		)
	...
	def output_dir(self):
		...
		output_dir = Path(__file__).parent.parent / ".test-results" / "e2e_haiku"
		output_dir.mkdir(parents=True, exist_ok=True)
		return output_dir

	def _get_model(self, provider: str, model_name: str, workflow: dict | None = None):
		...
		try:
			...
			for prev_node_id, prev_output in previous_outputs.items():
				if prev_output:
					...
					input_text = input_text.replace(placeholder, prev_output)
	...
	def test_node_execution_sequential(self, workflow, output_dir):
		...
		for i, (node_id, node_config) in enumerate(node_list, 1):
			...
			if error:
				...
				pytest.skip(f"Node {node_id} failed: {error}")
			else:
		...
		assert len(outputs) == 5, f"Not all nodes executed. Got {len(outputs)}/5"
	...
	def test_data_flow_through_pipeline(self, workflow, output_dir):
		...
		for node_id, output in outputs.items():
			if output:
				print(f" {node_id}: {len(output)} chars -> used by next node")
	...
	def test_error_handling(self, workflow):
		...
		output, error = self._execute_node("test_node", test_node, {})
	...
	def test_workflow_execution_with_trace_evidence(self, workflow, output_dir):
		...
		print("\n" + "=" * 80)
		print("Full Workflow Execution with Trace Evidence")
		print("=" * 80)
		...
		try:
			runner = WorkflowRunner(str(workflow_file), str(output_dir / ".hillstar"))
			result = runner.execute(resume_from=None)

			assert result is not None, "Execution returned None"
			assert result.get("status") == "success", f"Execution failed: {result.get('error')}"

			# Extract trace file path
			trace_file = result.get("trace_file")
			assert trace_file, "No trace file generated"

			print(f"\nTrace file: {trace_file}")
			print(f"Cost: ${result.get('cumulative_cost_usd', 0):.6f}")

			# Parse trace file for model call evidence
			model_calls = {}
				...
				with open(trace_file, "r") as f:
					for line in f:
						if line.strip():
							...
							if event.get("tool") == "model_call":
								node_id = event.get("node_id")
								if node_id not in model_calls:
									model_calls[node_id] = []
								model_calls[node_id].append({
			...
			# Generate execution evidence report
			evidence_file = output_dir / "execution_evidence.md"
			with open(evidence_file, "w") as f:
				f.write("# Execution Evidence - Model API Calls\n\n")
				f.write(f"Generated: {datetime.now().isoformat()}\n")
				f.write(f"Trace File: {trace_file}\n\n")

				for node_id in workflow["graph"]["nodes"].keys():
					if node_id in model_calls and model_calls[node_id]:
						for call in model_calls[node_id]:
							f.write(f"## {node_id}\n")
							f.write(f"- **Provider**: {call['provider']}\n")
							f.write(f"- **Model**: {call['model']}\n")
							f.write(f"- **Status**: {call['status']}\n")
							f.write(f"- **Tokens Used**: {call['tokens_used']}\n")
							f.write(f"- **Cost**: ${call['actual_cost_usd']:.6f}\n")
							f.write(f"- **Timestamp**: {call['timestamp']}\n\n")

			print(f"\nExecution Evidence Report: {evidence_file}")
			print(f"Model calls captured: {sum(len(calls) for calls in model_calls.values())}")

			# Assert we have evidence of model calls
			assert len(model_calls) > 0, "No model calls captured in trace file"
			print("SUCCESS: Model execution evidence verified")

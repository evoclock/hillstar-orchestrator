...
def test_schema_validation():
	...
	try:
		...
		log_event(
			"file_read",
			path="dev/examples/test-mcp/workflow.json",
...
def test_provider_registry():
	...
	try:
		...
		result_msg = f"PASS Registry test result: {found_count}/{total_count} MCP providers found"
...
def test_unit_tests():
	"""Test 4: Run existing pytest suite from correct directory."""
	logger.info("=" * 80)
	...
	try:
		...
		logger.info("Running: python -m pytest tests/ -v --co -q (test collection only)")
		log_event("command_execution", command="pytest tests/ --collect-only")

		# Just collect tests to verify test discovery works, don't run recursively
		...
		logger.info(f"Test collection exit code: {result.returncode}")
		log_event("command_complete", command="pytest --collect-only", exit_code=result.returncode)

		# Count tests collected
		lines = result.stdout.split("\n")
		collection_info = [line for line in lines if "test" in line.lower()]
		if collection_info:
			logger.info(f"Tests collected: {collection_info[-1]}")
			log_event("test_collection_info", info=collection_info[-1])

		if result.returncode == 0:
			logger.info("PASS Test collection successful (skipping recursive execution)")
			log_event("test_complete", test="unit_tests", result="pass")
			return True
		else:
			logger.warning("Test collection failed")
			logger.warning(result.stdout[-500:] if result.stdout else result.stderr[-500:])
			log_event("test_failure", output=result.stdout[-500:] if result.stdout else "")
			log_event("test_complete", test="unit_tests", result="fail")
			return False
...
def test_credential_leak_prevention_failure():
	...
	try:
		...
		log_event(
			"test_setup",
			api_key_length=len(mock_api_key),
...
"""
		...
		redacted_count_stderr = (
			result.stderr.count(mock_api_key) if result.stderr else 0
		)
		redacted_count_stdout = (
			result.stdout.count(mock_api_key) if result.stdout else 0
		)

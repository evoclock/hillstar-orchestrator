...
class TestLocalExecution:
	...
	def output_dir(self):
		...
		output_dir = Path(__file__).parent.parent / ".test-results" / "e2e_local"
	...
	def test_local_model_availability(self):
		...
		if not available:
			pytest.skip("Local Ollama not available at localhost:11434")
	...
	def test_local_model_instantiation(self, local_workflow):
		...
		try:
			...
			model_local = self._get_model(
				provider_local, model_name_local, local_workflow
			)

			assert model_local is not None
			msg = f" Successfully instantiated LOCAL: {provider_local}/{model_name_local}"
			print(msg)
			...
			model_cloud = self._get_model(
				provider_cloud, model_name_cloud, local_workflow
			)

			assert model_cloud is not None
			msg = f" Successfully instantiated CLOUD: {provider_cloud}/{model_name_cloud}"
			print(msg)
	...
	def test_local_model_execution(self, local_workflow, output_dir):
		...
		for node_id in ["analyze_local", "analyze_cloud"]:
			...
			try:
				...
				# Verify output
				if not output_text:
					print(f"[{node_type}] SKIPPED: Model returned empty output")
					continue
		...
		if len(results) == 0:
			pytest.skip("No models executed successfully (models may have returned empty output)")
	...
	def test_local_error_handling(self, local_workflow):
		...
		try:
			...
			result = model.call(prompt="test", max_tokens=10, temperature=0.0)
		...
		try:
			...
			model = self._get_model(
				"ollama_cloud", "nonexistent-cloud-model-xyz", local_workflow
			)
